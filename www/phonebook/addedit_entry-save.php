...
<?php
	...
	$save_entry->param( 'api_token',    $config_client['jsonrpc']['api_token'] );
	...
	if ( !empty( $_POST['entry_id'] )) {
		...
		$save_entry->param( 'entry_id', $_POST['entry_id'] );
	}
	...
	if ( !$result[ $save_entry->id ]['status'] ) {

		$_SESSION['errors'] = $result[ $save_entry->id ]['message'];
	}

	header( 'Location: /phonebook/entries.php?phonebook_id=' . $_POST['phonebook_id'] );
